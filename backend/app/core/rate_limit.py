        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if not user or not jwt_token:
                    logger.error(f"Rate limit decorator missing required parameters. user: {user is not None}, jwt_token: {jwt_token is not None}")
                    raise HTTPException(
                ...
                # Check rate limit
                result = await rate_limiter.check_rate_limit(user.id, action_type, jwt_token)
                ...
                if not rate_limit_data["allowed"]:
                    # Rate limit exceeded
                    logger.info(f"Rate limit exceeded for user {user.id}, action {action_type}")
                    raise HTTPException(
                        status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                        detail=f"Rate limit exceeded. Try again at {rate_limit_data['reset_at']}",
                        headers={
                            "X-RateLimit-Limit": str(rate_limit_data.get("remaining", 0) + 1),
                ...
                logger.debug(f"Rate limit check passed for user {user.id}, action {action_type}. Remaining: {rate_limit_data['remaining']}")

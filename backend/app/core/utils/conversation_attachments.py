...
class ConversationAttachmentsService(BaseDBService):
    ...
    def _convert_messages_to_langchain(
        ...
        for msg in raw_messages:
            ...
            if sender == "user":
                ...
                logger.warning(
                    f"Unknown sender type '{sender}' for message {msg.get('id')}"
                )

        return langchain_messages

    def _convert_bundles_to_workout_data(
        self, raw_bundles: List[Dict[str, Any]]
    ) -> List[UserContextBundle]:
        """Convert database bundle format to UserContextBundle objects"""
        workout_bundles = []
        ...
        for bundle_raw in raw_bundles:
            try:
                ...
                if bundle_raw.get("status") != "complete":
                    logger.info(
                        f"Skipping bundle {bundle_raw.get('id')} with status: {bundle_raw.get('status')}"
                    )
                    continue
                ...
                # Create UserContextBundle instance
                workout_bundle = UserContextBundle(**bundle_data)
    ...
    async def load_conversation_context(
        self, conversation_id: str, jwt_token: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = user_client.rpc(
                "get_conversation_attachments", {"p_conversation_id": conversation_id}
            ).execute()
    ...
    async def load_conversation_context_admin(
    ...
    async def _process_rpc_result(self, result, conversation_id: str) -> Dict[str, Any]:
        ...
        pending_bundles = [
            b for b in raw_bundles if b.get("status") in ["pending", "in_progress"]
        ]

        # Create context object
        context = ConversationContext(
            messages=langchain_messages, bundles=workout_bundles
        )

        logger.info(
            f"Successfully loaded {len(langchain_messages)} messages and {len(workout_bundles)} complete bundles for conversation {conversation_id}"
        )
        if pending_bundles:
...
async def load_conversation_context(
    conversation_id: str, jwt_token: str = None
) -> Dict[str, Any]:
    """

...
async def trigger_memory_extraction(user_id: str, conversation_id: str) -> None:
    """
    ...
    try:
        ...
        asyncio.create_task(memory_service.extract_memory(user_id, conversation_id))

        logger.info(
            f"✅ Memory extraction task created for conversation {conversation_id}"
        )

...
class AnalysisBundleProcessor:
    ...
    def process(
        ...
        try:
            ...
            logger.error(
                f"Critical error processing bundle {bundle_id}: {e}", exc_info=True
            )
            errors.append(f"Processing failed: {str(e)}")
            return self._create_failed_bundle(
                bundle_id, user_id, raw_workout_data, errors
            )

    def _build_metadata(
        self, raw_workout_data: Dict[str, Any], errors: list
    ) -> BundleMetadata:
    ...
    def _build_general_workout_data(
        self, workouts: list, errors: list
    ) -> GeneralWorkoutData:
            ...
            for workout in workouts:
                for exercise in workout.get("workout_exercises", []):
                    ...
                    if name:
                        ...
                        exercise_set_counts[name] = (
                            exercise_set_counts.get(name, 0) + num_sets
                        )
    ...
    def _build_recent_workouts(self, workouts: list, errors: list) -> list:
        """
    ...
    def _build_volume_data(self, workouts: list, errors: list) -> VolumeData:
        """
        ...
        try:
                ...
                for exercise in workout.get("workout_exercises", []):
                    ...
                    if exercise_name and exercise_volume > 0:
                        ...
                        exercise_date_volumes[exercise_name][date_key] = (
                            exercise_date_volumes[exercise_name].get(date_key, 0)
                            + exercise_volume
                        )
            ...
            for exercise_name, date_volumes in exercise_date_volumes.items():
                ...
                volume_by_exercise_over_time.append(
                    ExerciseVolumeData(exercise=exercise_name, time_series=time_series)
                )
    ...
    def _build_strength_data(self, workouts: list, errors: list) -> StrengthData:
        """
        ...
        try:
            ...
            for exercise_name, date_e1rms in exercise_date_e1rms.items():
                ...
                exercise_strength_progress.append(
                    ExerciseStrengthProgress(
                        exercise=exercise_name, e1rm_time_series=time_series
                    )
                )
            ...
            logger.info(
                f"Built strength data for {len(exercise_strength_progress)} exercises"
            )
    ...
    def _build_consistency_data(self, workouts: list, errors: list) -> ConsistencyData:
        """
        ...
        try:
            if len(workouts) < 2:
                return ConsistencyData(avg_days_between=0.0, variance=None)
            ...
            for workout in workouts:
                ...
                if date_str:
                    try:
                        ...
                        logger.warning(
                            f"Could not parse workout date '{date_str}': {e}"
                        )
        ...
        try:
            ...
            if exercise_definitions:
                for definition in exercise_definitions:
                    ...
                    if def_id:
                        definition_map[def_id] = (
                            primary_muscles if isinstance(primary_muscles, list) else []
                        )
            ...
            return MuscleGroupBalance(
                total_sets=true_total_sets, distribution=distribution
            )
    ...
    def _create_failed_bundle(
        self, bundle_id: str, user_id: str, raw_workout_data: Dict, errors: list
    ) -> UserContextBundle:
        ...
        return UserContextBundle(
            ...
            consistency_data=ConsistencyData(avg_days_between=0.0, variance=None),

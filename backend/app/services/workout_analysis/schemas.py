...
class BundleMetadata(BaseModel):
    ...
    bundle_type: str = Field(
        default="standard", description="Type of bundle (e.g., 'standard', 'advanced')"
    )
...
class GeneralWorkoutData(BaseModel):
    ...
    total_exercises_unique: int = Field(
        ..., description="Number of unique exercises performed"
    )
    date_range: DateRange = Field(..., description="Date range of workouts")
    exercises_included: List[str] = Field(
        default_factory=list, description="List of all unique exercise names"
    )
    exercise_frequency: ExerciseFrequency = Field(
        default_factory=ExerciseFrequency, description="Frequency metrics for exercises"
    )
...
class WorkoutSet(BaseModel):
    ...
    estimated_1rm: Optional[float] = Field(
        None, description="Estimated one-rep max calculated from weight × reps"
    )
    distance: Optional[float] = Field(
        None, description="Distance covered (for cardio exercises)"
    )
    duration: Optional[str] = Field(
        None, description="Duration of set in HH:MM:SS format (for timed exercises)"
    )
...
class WorkoutExercise(BaseModel):
    ...
    sets: List[WorkoutSet] = Field(
        default_factory=list, description="All sets performed for this exercise"
    )
...
class RecentWorkout(BaseModel):
    ...
    exercises: List[WorkoutExercise] = Field(
        default_factory=list, description="All exercises performed in this workout"
    )
...
class ExerciseVolumeTimeSeries(BaseModel):
    ...
    volume_kg: float = Field(
        ..., description="Total volume for this exercise on this date"
    )
...
class ConsistencyData(BaseModel):
    ...
    variance: Optional[float] = Field(
        None, description="Variance in workout frequency (null if insufficient data)"
    )
...
class MuscleGroupDistribution(BaseModel):
    ...
    muscle_group: str = Field(
        ..., description="Muscle group name (e.g., 'Chest', 'Back', 'Legs')"
    )
    percentage: float = Field(
        ..., description="Percentage of total muscle-set instances"
    )
...
class CorrelationInsights(BaseModel):
    ...
    significant_patterns: Optional[List[Dict]] = Field(
        None, description="List of significant patterns discovered in workout data"
    )
...
class UserContextBundle(BaseModel):
    ...
    user_profile: Optional[Dict[str, Any]] = Field(
        None, description="Full user profile"
    )
    ai_memory: Optional[Dict[str, Any]] = Field(
        None, description="Long-term memory extracted from conversations (note-based)"
    )

    # Core data sections
    metadata: BundleMetadata = Field(
        ..., description="Bundle metadata and generation info"
    )
    ...
    strength_data: StrengthData = Field(
        ..., description="Strength metrics and progression"
    )
    consistency_data: ConsistencyData = Field(
        ..., description="Frequency and consistency metrics"
    )

    # Optional/Premium sections
    muscle_group_balance: Optional[MuscleGroupBalance] = Field(
        None, description="Muscle group training distribution"
    )
    correlation_insights: Optional[CorrelationInsights] = Field(
        None, description="Advanced pattern analysis (premium only)"
    )

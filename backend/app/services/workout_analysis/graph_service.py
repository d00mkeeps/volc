    ...
    def _get_top_performers(self, workout_data: Dict, metric: str, limit: int = 3) -> List[Dict]:
        ...
        for workout in workout_data.get('workouts', []):
            ...
            for exercise in workout.get('exercises', []):
                # Try different field names and normalize case
                name = exercise.get('exercise_name', exercise.get('name', ''))
                if not name:
                    continue
                    
                # Normalize to title case (Dips, Face Pull, etc.)
                ...
                if normalized_name not in exercise_metrics:
                    exercise_metrics[normalized_name] = {'first': None, 'last': None, 'name': normalized_name}
                
                # Track first and last values for calculating change
                current_entry = {'date': workout_date, 'value': value}
                
                if not exercise_metrics[normalized_name]['first'] or workout_date < exercise_metrics[normalized_name]['first']['date']:
                    exercise_metrics[normalized_name]['first'] = current_entry
                if not exercise_metrics[normalized_name]['last'] or workout_date > exercise_metrics[normalized_name]['last']['date']:
                    exercise_metrics[normalized_name]['last'] = current_entry
        ...
        for exercise in top_exercises:
            ...
            for workout in workout_data.get('workouts', []):
                ...
                for ex in workout.get('exercises', []):
                    ...
                    ex_name = ex.get('exercise_name', ex.get('name', '')).strip().title()
                    if ex_name == exercise_name.strip().title():

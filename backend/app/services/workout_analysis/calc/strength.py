    ...
    def calculate(raw_workout_data: Dict, limit: int = 3) -> StrengthCalculatorResult:
        ...
        try:
            ...
            exercise_metrics, time_series_data = (
                StrengthCalculator._track_exercise_performance(workouts)
            )

            # Calculate improvements for top performers
            improvement_results = StrengthCalculator._calculate_improvements(
                exercise_metrics
            )

            # Sort by improvement and create top performers using schema
            improvement_results.sort(
            ...
            for exercise_name, data_points in time_series_data.items():
                time_series[exercise_name] = [
                    PerformanceDataPoint(
                        date=point["date"], highest_1rm=point["highest_1rm"]
                    )
                    for point in data_points
                ]

            logger.info(
                f"Found {len(improvement_results)} exercises with strength data, returning top {limit}"
            )
            ...
            for exercise in workout.get("exercises", []):
                ...
                if max_1rm > 0:
                    ...
                    time_series_data[f"{exercise_name} strength"].append(
                        {"date": workout_date, "highest_1rm": max_1rm}
                    )

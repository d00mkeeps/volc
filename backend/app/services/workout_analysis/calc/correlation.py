...
class CorrelationCalculator:
    ...
    @staticmethod
    def calculate(
        strength_time_series: Dict[str, List], volume_time_series: Dict[str, List]
    ) -> CorrelationCalculatorResult:
        ...
        try:
            ...
            if len(all_time_series) < 2:
                return CorrelationCalculatorResult(
                    ...
                    significant_count=0,
                    data_quality_notes=[
                        "Insufficient exercises for correlation analysis"
                    ],
                )

            # Convert to weekly series
            weekly_series, data_quality_notes = calc._convert_all_to_weekly_series(
                all_time_series
            )
            ...
            if len(weekly_series) < 2:
                return CorrelationCalculatorResult(
                    ...
                    + ["Insufficient weekly data for correlation analysis"],
                )

            # Analyze all pairs
            significant_correlations, total_pairs = calc._analyze_all_pairs(
                weekly_series
            )

            logger.info(
                f"Found {len(significant_correlations)} significant correlations out of {total_pairs} pairs analyzed"
            )
    ...
    def _convert_all_to_weekly_series(
        self, all_time_series: Dict
        ...
        for factor_name, data_points in all_time_series.items():
            ...
            try:
                ...
                for point in data_points:
                    ...
                    value = (
                        point.highest_1rm
                        if hasattr(point, "highest_1rm") and point.highest_1rm
                        else point.total_volume
                    )
                ...
                if len(weekly_series_data) >= 4:  # Need at least 4 weeks
                    ...
                    notes.append(
                        f"Insufficient weekly data for {factor_name} ({len(weekly_series_data)} weeks, need 4+)"
                    )
    ...
    def _analyze_all_pairs(
        self, weekly_series: Dict[str, pd.Series]
    ) -> tuple[List[SignificantCorrelation], int]:
        ...
        for i, factor1 in enumerate(factors):
            for j, factor2 in enumerate(factors):
                ...
                lag_results = self._time_lagged_correlation(
                    weekly_series[factor1], weekly_series[factor2]
                )
                ...
                if strongest["significant"]:
                    ...
                    outcome, predictor = self._determine_outcome_predictor(
                        factor1, factor2, strongest
                    )

                    correlation = SignificantCorrelation(
                        outcome=outcome,
                        predictor=predictor,
                        strength=self._categorize_strength(
                            abs(strongest["correlation"])
                        ),
                        direction=(
                            "positive" if strongest["correlation"] > 0 else "negative"
                        ),
    ...
    def _determine_outcome_predictor(
        self, factor1: str, factor2: str, strongest: Dict
    ) -> tuple[str, str]:

...
class AnalysisBundleGenerator:
    ...
    async def generate_analysis_bundle(
        ...
        try:
            ...
            if not bundle_result.get("success"):
                ...
                return {"success": False, "error": f"Failed to create bundle: {error}"}
            ...
            if not raw_result or not raw_result.get("success"):
                error = (
                    raw_result.get("error", "No workout data returned")
                    if raw_result
                    else "No workout data"
                )
                logger.warning(
            ...
            logger.info(
                f"✅ Loaded {len(exercise_definitions)} exercise definitions from cache"
            )
            ...
            if memory_result.data and memory_result.data.get("ai_memory"):
                ...
                logger.info(
                    f"🧠 Preserving existing ai_memory with {len(existing_ai_memory.get('notes', []))} notes"
                )
            else:
            ...
            logger.info(
                f"🔬 Processing {workout_count} workouts through AnalysisBundleProcessor"
            )
            ...
            logger.info(
                f"✅ Processing complete. Bundle status: {complete_bundle.status}"
            )
            ...
            logger.error(
                f"💥 Critical error generating analysis bundle: {str(e)}", exc_info=True
            )
            ...
            return {"success": False, "error": str(e), "bundle_id": bundle_id}

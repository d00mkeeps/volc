...
class PerformanceProfiler:
    ...
    def start_phase(self, name: str, **metadata) -> PerformancePhase:
        ...
        phase = PerformancePhase(name=name, start_time=time.time(), metadata=metadata)
    ...
    def _log_warnings(self, summary: Dict):
        ...
        for phase_data in summary["phases"]:
            ...
            if name == "context_loading" and duration > 0.5:
                logger.warning(
                    f"⚠️  Context loading slow: {duration:.3f}s (target: <0.5s)"
                )

            # User profile warnings
            elif name == "user_profile_loading" and duration > 0.1:
                logger.warning(
                    f"⚠️  User profile loading slow: {duration:.3f}s (target: <0.1s)"
                )

            # Prompt formatting warnings
            elif name == "prompt_formatting" and duration > 0.2:
                logger.warning(
                    f"⚠️  Prompt formatting slow: {duration:.3f}s (target: <0.2s)"
                )

            # LLM first token warnings
            elif name == "llm_first_token" and duration > 2.0:
                logger.warning(
                    f"⚠️  Time to first token high: {duration:.3f}s (target: <2.0s)"
                )

            # Agent LLM call warnings
            elif name.startswith("agent_llm_call"):
                if duration > 6.0:
                    logger.warning(f"⚠️  {name} slow: {duration:.3f}s (target: <6.0s)")
                ...
                if prompt_size > 50000:
                    ...
                    logger.info(f"ℹ️  {name} moderate context: {prompt_size:,} chars")
                ...
                if result_type == "tool_calls":
                    ...
                    logger.info(
                        f"ℹ️  {name} generated final answer ({response_length} chars)"
                    )
        ...
        agent_llm_calls = [
            p for p in summary["phases"] if p["name"].startswith("agent_llm_call")
        ]
        ...
        # Count tool calls
        tool_calls = [
            p for p in summary["phases"] if p["name"].startswith("tool_execution")
        ]
        if len(tool_calls) > 3:
            logger.warning(

...
class ChatActionService:
    ...
    async def generate_actions(
        self, user_id: str, messages: List[Dict[str, str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"   - AI Memory: {ai_memory_info[:100]}..."
                if len(ai_memory_info) > 100
                else f"   - AI Memory: {ai_memory_info}"
            )
            if messages:
                logger.info(
                    f"   - Conversation Context: {len(messages)} messages provided"
                )
                if len(messages) > 0:
                    logger.info(
                        f"   - Last Message: {messages[-1].get('content', '')[:50]}..."
                    )
            else:
            ...
            messages_to_send = [
                SystemMessage(
                    content="You are a helpful fitness coach assistant. Generate JSON only."
                ),
            ...
            if not actions:
                logger.warning(
                    f"⚠️ Empty actions generated for user {user_id}, using defaults"
                )
                    ...
                    actions = [
                        {
                            ...
                            "message": "Let's continue with my workout",
                        },
                        {
                            "label": "End session",
                            "message": "I'd like to end this workout session",
                        },
                        {
                            "label": "Ask question",
                            "message": "I have a question about my training",
                        },
                    ]

            return {"actions": actions[:3]}  # Ensure max 3

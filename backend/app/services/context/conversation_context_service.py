    ...
    async def load_context(self, conversation_id: str, jwt_token: str, user_id: Optional[str] = None) -> ConversationContext:
        ...
        try:
            ...
            if self._is_cached_and_valid(conversation_id):
                ...
                logger.info(f"⚡ CACHE HIT: Context loaded in {elapsed:.2f}s for conversation: {conversation_id}")
            ...
            logger.info(f"✅ UNIFIED SERVICE: Context loaded successfully in {total_elapsed:.2f}s - {len(context.messages)} messages, {len(context.bundles)} bundles")

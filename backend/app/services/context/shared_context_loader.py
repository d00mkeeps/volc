...
class SharedContextLoader:
    ...
    async def load_all(self, user_id: str) -> Dict[str, Any]:
        ...
        results = await asyncio.gather(
            profile_task, bundle_task, glossary_task, return_exceptions=True
        )
        ...
        if isinstance(profile_result, Exception):
            ...
            isinstance(profile_result, dict)
            and profile_result.get("success")
            and profile_result.get("data")
        ):
            ...
            logger.warning(
                f"⚠️ Profile load returned success=False or no data: {profile_result}"
            )
        ...
        if isinstance(bundle_result, Exception):
            ...
            isinstance(bundle_result, dict)
            and bundle_result.get("success")
            and bundle_result.get("data")
        ):
            ...
            logger.warning(
                f"⚠️ Bundle load returned success=False or no data: {bundle_result}"
            )
        ...
        if isinstance(glossary_result, Exception):
            ...
            logger.warning(
                f"⚠️ Glossary load returned unexpected format: {type(glossary_result)}"
            )

        # Update cache
        self._cache[user_id] = {"data": context, "timestamp": time.time()}

...
# Update the existing method to handle empty JWTs gracefully:
        ...
        try:
            ...
            if not jwt_token or jwt_token.strip() == "":
                ...
                if user_id:
                    ...
                    result = admin_client.table("user_profiles").select("permission_level").eq("auth_user_uuid", user_id).execute()
            ...
            if permission_level and permission_level in [PermissionLevels.TESTER, PermissionLevels.ADMIN]:
                return permission_level
            
            # Fallback: Look up from database if not in JWT
            if user_id:
                logger.info(f"Permission level not in JWT, looking up from database for user {user_id}")
                admin_client = self.get_admin_client()
                result = admin_client.table("user_profiles").select("permission_level").eq("auth_user_uuid", user_id).execute()
    ...
    async def check_rate_limit(self, user_id: str, action_type: str, jwt_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = admin_client.table("user_rate_limits").select("*").eq("user_id", user_id).eq("action_type", action_type).execute()
            ...
            if current_time >= window_start + window_duration:
                ...
                admin_client.table("user_rate_limits").update(updated_record).eq("id", record["id"]).execute()

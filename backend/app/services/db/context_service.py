...
class ContextBundleService:
    ...
    def _default_consistency_data(self) -> Dict[str, Any]:
        ...
        return {"avg_days_between": 0.0, "variance": None}
    ...
    def _normalize_ai_memory(self, ai_memory: Any) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(ai_memory, list):
            logger.warning(
                f"Found corrupted ai_memory (list instead of dict), normalizing"
            )
    ...
    def _deserialize_bundle(self, db_row: Dict[str, Any]) -> "UserContextBundle":
        ...
        flat_bundle = {
            ...
            or self._default_general_workout_data(),
        ...
        try:
            ...
            if latest_bundle_result.get("success") and latest_bundle_result.get("data"):
                ...
                if latest_bundle.ai_memory:
                    ...
                    logger.info(
                        f"Copying memory from previous bundle for user {user_id}"
                    )
            ...
            if hasattr(result, "data") and result.data:
                logger.info(
                    f"Analysis bundle created successfully with ID: {bundle_id}"
                )
                return {"success": True, "data": {"bundle_id": bundle_id}}
            else:
    ...
    async def update_bundle_status(
        ...
        try:
            ...
            if hasattr(result, "data") and result.data:
                logger.info(
                    f"Bundle status updated successfully: {bundle_id} -> {status}"
                )
                return {"success": True}
            else:
    ...
    async def save_context_bundle_admin(
        self, bundle_id: str, bundle: Any  # UserContextBundle
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error saving analysis bundle (admin): {str(e)}", exc_info=True
            )
            return {"success": False, "error": str(e)}

    async def update_ai_memory_admin(
        self, bundle_id: str, ai_memory: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def append_ai_memory_admin(
        self, bundle_id: str, new_notes: list[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(update_result, "data") and update_result.data:
                logger.info(
                    f"✅ Appended notes successfully, total: {len(unique_notes)}"
                )
                return {"success": True, "note_count": len(unique_notes)}
            else:
                return {"success": False, "error": "Failed to save updated memory"}
    ...
    async def get_latest_context_bundle(
        ...
        try:
            ...
            if hasattr(result, "data") and result.data:
                ...
                return {"success": False, "error": "No bundle found"}

        except Exception as e:
            logger.error(f"Error fetching analysis bundle: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}

    async def get_latest_context_bundle_admin(self, user_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                f"Error fetching analysis bundle (admin): {str(e)}", exc_info=True
            )
            return {"success": False, "error": str(e)}
    ...
    async def delete_old_user_context_bundles(
        ...
        try:
            ...
            logger.info(
                f"Deleted {deleted_count} old analysis bundles for user: {user_id}"
            )
    ...
    async def append_onboarding_notes(
        self, user_id: str, notes: list[Dict[str, str]], jwt_token: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result.data and len(result.data) > 0:
                ...
                return (
                    await self.format_response(update_result.data[0])
                    if hasattr(self, "format_response")
                    else {"success": True, "data": update_result.data[0]}
                )
            else:
                # Create new bundle
                from uuid import uuid4
                ...
                logger.info(
                    f"Created new bundle with onboarding notes for user {user_id}"
                )
                return (
                    await self.format_response(insert_result.data[0])
                    if hasattr(self, "format_response")
                    else {"success": True, "data": insert_result.data[0]}
                )

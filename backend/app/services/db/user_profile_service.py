...
class UserProfileService(BaseDBService):
    ...
    async def save_user_profile_admin(
        ...
        try:
            ...
            if "completed_onboarding_tour" in profile_data:
                user_profile["completed_onboarding_tour"] = profile_data[
                    "completed_onboarding_tour"
                ]
            ...
            if profile_exists:
                ...
                logger.info(
                    f"No existing profile found for user {user_id}, creating new profile"
                )
                user_profile["auth_user_uuid"] = user_id

                result = (
                    admin_client.table("user_profiles").insert(user_profile).execute()
                )
            ...
            return {"success": True, "data": profile_result.data[0]}

        except Exception as e:
            logger.error(
                f"Error saving user profile (admin) for {user_id}: {str(e)}",
                exc_info=True,
            )
            return {"success": False, "error": str(e)}

    async def save_user_profile(
        self, user_id: str, profile_data: Dict[str, Any], jwt_token: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not hasattr(result, "data") or not result.data:
                ...
                logger.info(
                    f"No existing profile found for user {user_id}, attempting to create"
                )
                ...
                result = (
                    user_client.table("user_profiles").insert(user_profile).execute()
                )
            ...
            profile_result = user_client.table("user_profiles").select("*").execute()
    ...
    async def get_user_profile(self, user_id: str, jwt_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            user_client = self.get_user_client(jwt_token)
            result = user_client.table("user_profiles").select("*").execute()
    ...
    async def delete_user_account(self, user_id: str, jwt_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = user_client.rpc(
                "delete_user_account", {"user_uuid": user_id}
            ).execute()
    ...
    async def get_user_profile_admin(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            response = (
                ...
                .table("user_profiles")
                .select("user_id, first_name, last_name, is_imperial")
                .eq("auth_user_uuid", user_id)
                .execute()
            )
    ...
    async def complete_onboarding(
        self, user_id: str, onboarding_data: Dict[str, Any], jwt_token: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"Completing onboarding for user {user_id} with data: {onboarding_data}"
            )
            ...
            logger.info(
                f"Appending {len(notes)} notes to context bundle for user {user_id}"
            )
            await context_bundle_service.append_onboarding_notes(
                user_id, notes, jwt_token
            )

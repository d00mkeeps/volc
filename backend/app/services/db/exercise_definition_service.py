...
class ExerciseDefinitionService(BaseDBService):
    ...
    async def get_all_exercise_definitions(self, jwt_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = (
                user_client.table("exercise_definitions")
                .select("*")
                .eq("is_active", True)
                .order("standard_name")
                .execute()
            ...
            if hasattr(result, "error") and result.error:
                raise Exception(
                    f"Failed to fetch exercise definitions: {result.error.message}"
                )
    ...
    async def get_all_exercise_definitions_admin(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.data:
                logger.info(
                    f"Successfully loaded {len(response.data)} exercise definitions"
                )
                return {"success": True, "data": response.data}
            else:
                logger.warning("No exercise definitions found")
                return {"success": False, "error": "No exercise definitions found"}

        except Exception as e:
            logger.error(
                f"Error fetching exercise definitions: {str(e)}", exc_info=True
            )
            return {"success": False, "error": str(e)}

    async def create_exercise_definition(
        self, exercise_data: Dict[str, Any], jwt_token: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"Creating new exercise definition: {exercise_data.get('standard_name')}"
            )
            ...
            if hasattr(result, "error") and result.error:
                raise Exception(
                    f"Failed to create exercise definition: {result.error.message}"
                )
            ...
            logger.info(
                f"Exercise definition created with ID: {created_definition.get('id')}"
            )
    ...
    async def get_exercise_definition_by_id(
        self, definition_id: str, jwt_token: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if hasattr(result, "error") and result.error:
                raise Exception(
                    f"Failed to fetch exercise definition: {result.error.message}"
                )
            ...
            logger.info(
                f"Retrieved exercise definition: {definition.get('standard_name')}"
            )

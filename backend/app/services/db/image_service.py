...
class ImageService(BaseDBService):
    ...
    async def create_temp_image(
        self, user_id: str, file_extension: str, jwt_token: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            image_result = (
                user_client.table("images").insert(image_insert_data).execute()
            )
            ...
            signed_url = user_client.storage.from_(
                self.bucket_name
    ...
    async def get_image_url(
        self, image_id: str, user_id: str, jwt_token: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if image_owner_id == user_id:
                ...
                logger.info(
                    "User doesn't own image - checking if image belongs to any workout"
                )
                ...
                logger.info(
                    "Image belongs to a workout - allowing access (all workouts are public)"
                )
            ...
            user_client = self.get_user_client(jwt_token)
            url_result = user_client.storage.from_(self.bucket_name).get_public_url(
                file_path
            )
            ...
            if isinstance(url_result, str):
                ...
                if "publicUrl" in url_result:
                    ...
                    logger.error(
                        f"Unknown structure - available keys: {list(url_result.keys())}"
                    )

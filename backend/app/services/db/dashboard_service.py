...
class DashboardService(BaseDBService):
    ...
    async def get_dashboard_data(self, user_id: str, jwt_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            workouts_result = await self.workout_service.get_user_workouts(
                user_id, jwt_token
            )

            if not workouts_result.get("success"):
                raise Exception(
                    f"Failed to get workouts: {workouts_result.get('error')}"
                )
            ...
            logger.info(
                f"📊 STEP 1 - Raw from service: {len(workouts)} workouts, {total_exercises} exercises, {total_sets} sets"
            )
            ...
            filtered_exercises = sum(
                len(w.get("workout_exercises", [])) for w in recent_workouts
            )
            filtered_sets = sum(
                len(ex.get("workout_exercise_sets", []))
            ...
            logger.info(
                f"📊 STEP 2 - After 60-day filter: {len(recent_workouts)} workouts, {filtered_exercises} exercises, {filtered_sets} sets"
            )

            logger.info(
                f"Processing {len(recent_workouts)} workouts for dashboard analytics"
            )
            ...
            for timeframe, data in dashboard_data.items():
                if timeframe != "lastUpdated":
                    ...
                    logger.info(
                        f"📊 STEP 3 - {timeframe}: {data['consistency']['totalWorkouts']} workouts, muscle_sets: {total_muscle_sets}"
                    )

            logger.info(
                f"Successfully processed dashboard data for user: {user_id}  Final dashboard data structure: {dashboard_data}"
            )
    ...
    def _calculate_timeframe_data(
        self, workouts: List[Dict], days: int
    ) -> Dict[str, Any]:
        ...
        actual_exercises = sum(
            len(w.get("workout_exercises", [])) for w in timeframe_workouts
        )
        actual_sets = sum(
            len(ex.get("workout_exercise_sets", []))
    ...
    def _get_all_exercise_definitions(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            result = (
                admin_client.table("exercise_definitions")
            ...
            if hasattr(result, "error") and result.error:
                logger.warning(
                    f"Error batch fetching definitions: {result.error.message}"
                )
                return {}
    ...
    def _calculate_muscle_balance(self, workouts: List[Dict]) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            for group, sets in sorted(
                muscle_sets.items(), key=lambda x: x[1], reverse=True
            )
    ...
    def _calculate_consistency(self, workouts: List[Dict], days: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "workouts": [{"id": w["id"], "date": w["created_at"]} for w in workouts],
        }

...
async def verify_admin_test_key(x_admin_key: str = Header(None)):
    """
    Security check for test admin endpoints.
    - Only enabled in non-production environments.
    - Requires X-Admin-Key header matching ADMIN_TEST_KEY env var.
    """
    if os.environ.get("APP_ENV") == "production":
        # Silently fail in production to avoid exposing endpoint existence
        raise HTTPException(status_code=404, detail="Not Found")
    
    from dotenv import load_dotenv
    load_dotenv(override=True)
    
    expected_key = os.environ.get("ADMIN_TEST_KEY")
    if not expected_key or x_admin_key != expected_key:
        logger.warning(f"Unauthorized Admin Test access attempt")
...
async def get_admin_stats(user=Depends(get_current_user)):
    ...
    try:
        ...
        if (
            not profile_response.data
            or profile_response.data.get("permission_level") != "admin"
        ):
        ...
        error_rate = (
            round((error_count / total_requests) * 100, 1) if total_requests > 0 else 0
        )

        # Group by endpoint
        endpoint_stats = defaultdict(
            lambda: {"count": 0, "latency_sum": 0, "history": []}
        )
        ...
        for log in logs:
            ...
            endpoint_stats[path]["history"].append(
                {"timestamp": log["timestamp"], "latency": log["latency_ms"]}
            )
            ...
            top_endpoints.append(
                {
                    ...
                    "history": stats["history"][
                        :50
                    ],  # Limit history points sent to frontend
                }
            )
...
async def get_llm_stats(user=Depends(get_current_user)):
    ...
    try:
        ...
        input_tokens_list = [
            log["input_tokens"] for log in logs if log.get("input_tokens")
        ]
        output_tokens_list = [
            log["output_tokens"] for log in logs if log.get("output_tokens")
        ]
        ...
        token_stats = {
            ...
            "avg_input_tokens": (
                int(sum(input_tokens_list) / len(input_tokens_list))
        ...
        raise HTTPException(status_code=500, detail=str(e))
@router.post("/api/admin/test/provision")
async def provision_test_user(
    persona_config: Dict[str, Any],
    _=Depends(verify_admin_test_key)
):
    """
    Provision a fresh test user with profile and AI memory.
    """
    result = await test_account_service.setup_complete_test_account(
        email=persona_config.get("email"),
        password=persona_config.get("password"),
        profile=persona_config.get("profile", {}),
...
@router.delete("/api/admin/test/teardown/{email}")
async def teardown_test_user(
    email: str,
    _=Depends(verify_admin_test_key)
):
    """
    Delete a test user and all associated data by email.
    """
    result = await test_account_service.delete_user_by_email(email)

...
class TestAccountService:
    ...
    async def create_conversation_for_user(
        ...
        try:
            ...
            result = admin_client.table("conversations").insert({
                "user_id": user_id,
                "title": title,
    ...
    async def seed_user_profile(
        ...
        try:
            ...
            # Upsert profile
            result = admin_client.table("user_profiles").upsert(
                profile_data,
                on_conflict="auth_user_uuid"
            ).execute()
    ...
    async def seed_ai_memory(
        ...
        try:
            ...
            notes_with_dates = [
                {**note, "date": current_date} for note in notes
            ]
            
            # Check for existing bundle
            existing = admin_client.table("user_context_bundles") \
                .select("id, ai_memory") \
                .eq("user_id", user_id) \
            ...
            if existing.data:
                ...
                # Create new bundle
                bundle_id = str(uuid4())
                result = admin_client.table("user_context_bundles").insert({
                    "id": bundle_id,
                    "user_id": user_id,
    ...
    async def delete_test_user(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                convs = admin_client.table("conversations").select("id").eq("user_id", user_id).execute()
                if convs.data:
                    conv_ids = [c["id"] for c in convs.data]
                    for conv_id in conv_ids:
                        admin_client.table("messages").delete().eq("conversation_id", conv_id).execute()
            ...
            try:
                admin_client.table("conversations").delete().eq("user_id", user_id).execute()
            ...
            try:
                admin_client.table("user_context_bundles").delete().eq("user_id", user_id).execute()
            ...
            try:
                admin_client.table("user_profiles").delete().eq("auth_user_uuid", user_id).execute()
            ...
            try:
                admin_client.table("user_rate_limits").delete().eq("user_id", user_id).execute()
    ...
    async def reset_persona(
        ...
        """
        from ..personas.config import get_persona, get_iteration, increment_iteration
        
        # Get current persona config (before increment)
        current_persona = get_persona(persona_name)
        if not current_persona:
            return {"success": False, "error": f"Unknown persona: {persona_name}"}
        
        # Delete existing account if it exists
        logger.info(f"Deleting existing {persona_name} account: {current_persona['email']}")
    ...
    async def reset_all_personas(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        from ..personas.config import list_personas, increment_iteration

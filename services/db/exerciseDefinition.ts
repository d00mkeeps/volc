...
export class ExerciseDefinitionService extends BaseDBService {
  /**
  ...
  async getAllExerciseDefinitions(): Promise<ExerciseDefinition[]> {
    try {
      ...
      const data = await apiGet<ExerciseDefinition[]>(
        "/db/exercise-definitions"
      );

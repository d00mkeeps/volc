...
export const useGlossaryStore = create<GlossaryStoreState>()(
  persist(
    (set, get) => {
    ...
    },
    {
      ...
      partialize: (state) => ({
        terms: state.terms,
        termsById: state.termsById,
        // we omit complex state like sets, but actually dismissedTermIds is a Set which needs serialization
        // persist handles basic JSON, so Set needs custom serialize or we just rely on glossaryPersistence sqlite
      }),
    },
  ),
);

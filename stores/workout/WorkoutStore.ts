      ...
      return {
            ...
            set({
              error:
                err instanceof Error
                  ? err
                  : new Error("Failed to load workout"),
            });
          } finally {
            set({ loading: false });
          }
          ...
          try {
            ...
            set({
              error:
                err instanceof Error
                  ? err
                  : new Error("Failed to initiate save"),
            });
          } finally {
        ...
        syncPendingWorkouts: async () => {
          ...
          for (const item of pending) {
            ...
            try {
              ...
              if (!session?.user?.id) {
                console.log(
                  "[WorkoutStore] ⚠️ No session found, skipping sync",
                );
                continue;
              }
          ...
          try {
            ...
            set({
              error:
                err instanceof Error
                  ? err
                  : new Error("Failed to update workout"),
            });
            throw err;
          } finally {
            set({ loading: false });
        ...
        fetchTemplates: () => {
          ...
          const templates = workouts.filter(
            (workout) =>
              workout.workout_exercises.length > 0 && workout.is_template,
          );
          set({ templates });
        },
      };

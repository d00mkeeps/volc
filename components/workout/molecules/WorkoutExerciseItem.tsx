...
const WorkoutExerciseItem: React.FC<WorkoutExerciseItemProps> = ({
  ...
  const getHeaderConfig = () => {
    const config = {
      ...
      distance: exercise.workout_exercise_sets.some(
        (set) => set.distance !== null
      ),
      duration: exercise.workout_exercise_sets.some(
  ...
  if (headerConfig.weight)
    headerItems.push(`Weight (${exercise.weight_unit || "kg"})`);
  if (headerConfig.reps) headerItems.push("Reps");
  if (headerConfig.rpe) headerItems.push("RPE");
  if (headerConfig.distance)
    headerItems.push(`Distance (${exercise.distance_unit || "m"})`);
